...
sub get_osx_vers {
	...
	if ($sw_vers != $darwin_osx) {
		# Special cases in Big Sur, Monterey, Ventura, Sonoma, 
		# Sequoia, and Tahoe where it's OK to have a mismatch.
		my %sw_to_darwin = (
			'11.6' => 11.5,
			'11.7' => 11.5,
			'12.6' => 12.5,
...
sub get_darwin_equiv {
	...
	} elsif ($kernel_vers >= 27) {
		# darwin27.0 == 27.0 (beta)
		return $darwin_osx{$kernel_vers} || ($kernel_vers) . '.' . ($kernel_vers_minor);
	}
}
